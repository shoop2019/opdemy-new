...
class Meow_wr3x_Dashboard {
	...
	function admin_menu_dashboard () {
		...
		$warning_title = __( "Retina images", 'wp-retina-3x' );
		...
		add_media_page( 'Retina', $menu_label, 'manage_options', 'wp-retina-3x', array( $this, 'dashboard' ) );
	}
	...
	function dashboard() {
		...
		if ( $ignore ) {
			if ( !$this->core->admin->is_registered() ) {
				...
				_e( "Ignore is a Pro feature.", 'wp-retina-3x' );
				echo "</p></div>";
			}
			else
		...
		if ( $clearlogs ) {
			if ( file_exists( plugin_dir_path( __FILE__ ) . '/wp-retina-3x.log' ) ) {
				unlink( plugin_dir_path( __FILE__ ) . '/wp-retina-3x.log' );
			}
		}
		...
		echo '<div class="wrap">';
	  echo $this->core->admin->display_title( "WP Retina 3x" );
		echo '<p></p>';
		...
		<div style='background: #FFF; padding: 5px; border-radius: 4px; height: 28px; box-shadow: 0px 0px 6px #C2C2C2;'>
			...
			<!-- REFRESH -->
			<a id='wr3x_refresh' href='?page=wp-retina-3x&view=issues&refresh=true' class='button' style='float: left;'><span style="top: 3px; position: relative; left: -5px;" class="dashicons dashicons-controls-repeat"></span><?php _e("Refresh", 'wp-retina-3x'); ?></a>
			...
			<form id="posts-filter" action="upload.php" method="get">
				<p class="search-box" style='margin-left: 5px; float: left;'>
					...
					<input type="hidden" name="page" value="wp-retina-3x">
			...
			<!-- REMOVE BUTTON ALL -->
			<a id='wr3x_remove_button_all' onclick='wr3x_delete_all()' class='button button-red' style='float: right;'><span style="top: 3px; position: relative; left: -5px;" class="dashicons dashicons-trash"></span><?php _e("Bulk Delete (Retina Only)", 'wp-retina-3x'); ?></a>

			<!-- GENERATE ALL -->
			<a id='wr3x_generate_button_all' onclick='wr3x_generate_all()' class='button-primary' style='float: right; margin-right: 5px;'><span style="top: 3px; position: relative; left: -5px;" class="dashicons dashicons-controls-play"></span><?php _e("Bulk Generate (Thumbnails & Retina)", 'wp-retina-3x'); ?></a>


			<!-- PROGRESS -->
			<span style='margin-left: 12px; font-size: 13px; top: 5px; position: relative; color: #24547C; font-weight: bold;' id='wr3x_progression'></span>
		...
		<?php
			if (isset ( $_GET[ 'clearlogs' ] ) ? $_GET[ 'clearlogs' ] : 0) {
				...
				_e( "The logs have been cleared.", 'wp-retina-3x' );
				echo "</p></div>";
			}

			$active_sizes = $this->core->get_active_image_sizes();
			$full_size_needed = get_option( "wr3x_full_size" );
		...
		<p>
			<?php printf( __( 'Based on your <i>image sizes</i> settings, the full-size images should be uploaded at a resolution of at least <b>%d×%d</b> for the plugin to be able generate the <b>%d retina images</b>. Please note that it vares depending on your needs for each image (you will need to discuss this with your developer).', 'wp-retina-3x' ), $max_width, $max_height, count( $active_sizes ) ); ?>
			<?php if ( $full_size_needed ) printf( __(  "You <b>also need</b> to upload a retina image for the Full-Size image (might be <b>%d×%d</b>).", 'wp-retina-3x' ), $max_width * 2, $max_height * 2 ); ?>
			<?php _e("You can upload or replace the images by drag & drop.", 'wp-retina-3x' ); ?>
			<?php printf( __( "Your PHP configuration allows uploads of <b>%dMB</b> maximum.", 'wp-retina-3x'), $upload_max_size / 1000000 ); ?>

			<?php
				if ( file_exists( plugin_dir_path( __FILE__ ) . '/wp-retina-3x.log' ) ) {
					printf( __( 'The <a target="_blank" href="%s/wp-retina-3x.log">log file</a> is available. You can also <a href="?page=wp-retina-3x&view=issues&clearlogs=true">clear</a> it.', 'wp-retina-3x' ), plugin_dir_url( __FILE__ ) );
				}
			?>
		</p>
		...
		echo paginate_links(array(
		  'base' => '?page=wp-retina-3x&s=' . urlencode($s) . '&view=' . $view . '%_%',
	      'current' => $paged,
	      'format' => '&paged=%#%',
	      'total' => $pagescount,
	      'prev_next' => false
		...
		<ul class="subsubsub">
			<li class="all"><a <?php if ( $view == 'all' ) echo "class='current'"; ?> href='?page=wp-retina-3x&s=<?php echo $s; ?>&view=all'><?php _e( "All", 'wp-retina-3x' ); ?></a><span class="count">(<?php echo $totalcount; ?>)</span></li> |

			<?php if ( $this->core->admin->is_registered() ): ?>

			<li class="all"><a <?php if ( $view == 'issues' ) echo "class='current'"; ?> href='?page=wp-retina-3x&s=<?php echo $s; ?>&view=issues'><?php _e( "Issues", 'wp-retina-3x' ); ?></a><span class="count">(<?php echo $issues_count; ?>)</span></li> |
			<li class="all"><a <?php if ( $view == 'ignored' ) echo "class='current'"; ?> href='?page=wp-retina-3x&s=<?php echo $s; ?>&view=ignored'><?php _e( "Ignored", 'wp-retina-3x' ); ?></a><span class="count">(<?php echo count( $ignored ); ?>)</span></li>

			<?php else: ?>

			<li class="all"><span><?php _e( "Issues", 'wp-retina-3x' ); ?></span> <span class="count">(<?php echo $issues_count; ?>)</span></li> |
			<li class="all"><span><?php _e( "Ignored", 'wp-retina-3x' ); ?></span> <span class="count">(<?php echo count( $ignored ); ?>)</span></li>

			<?php endif; ?>


		</ul>
		<table class='wp-list-table widefat fixed media wr3x-table'>
			<thead><tr>
				<?php
				echo "<th style='width: 56px;'>Thumbnail</th>";
				echo "<th style=' width: 360px;'>" . __( "Base image", 'wp-retina-3x' ) . "</th>";
				echo "<th style=''>" . __( "Media Sizes<br />Retina-ized", 'wp-retina-3x' ) . "</th>";
				echo "<th style=''>" . __( "Full-Size<br/><b>Replace</b>", 'wp-retina-3x' ) . "</th>";
				echo "<th style=''>" . __( "Full-Size Retina", 'wp-retina-3x' ) . "</th>";
				echo "<th style=''>" . __( "Full-Size Retina<br/><b>Upload</b>", 'wp-retina-3x' ) . "</th>";
				?>
			</tr></thead>
			<tbody>
				<?php
				foreach ($results as $index => $attr) {
					...
					echo "<tr class='wr3x-file-row' postId='" . $post->ID . "'>";
					...
					if ( !$original_width || !$original_height ) {
						...
						echo "<td class='wr3x-image wr3x-info-thumbnail'><img src='" . $attachmentsrc[0] . "' /></td>";
						echo "<td class='wr3x-title'><a href='media.php?attachment_id=" . $post->ID . "&action=edit'>" . ( $post->post_title ? $post->post_title : '<i>Untitled</i>' ) . '</a><br />' .
						...
						echo "<a style='position: relative;' onclick='wr3x_generate(" . $post->ID . ", true)' id='wr3x_generate_button_" . $post->ID . "' class='wr3x-button'>" . __( "GENERATE", 'wp-retina-3x' ) . "</a>";
						if ( !$this->core->is_ignore( $post->ID ) )
							echo " <a href='?page=wp-retina-3x&view=" . $view . "&paged=" . $paged . "&ignore=" . $post->ID . "' id='wr3x_generate_button_" . $post->ID . "' class='wr3x-button wr3x-button-ignore'>" . __( "IGNORE", 'wp-retina-3x' ) . "</a>";
						echo " <a style='position: relative;' class='wr3x-button wr3x-button-view'>" . __( "DETAILS", 'wp-retina-3x' ) . "</a>";
						echo "</div></td>";
					}

					// Media Sizes Retina-ized
					echo '<td id="wr3x-info-' . $post->ID . '" style="padding-top: 10px;" class="wr3x-info">';
					if ( $original_width && $original_height )
					...
					if ( $this->core->admin->is_registered() ) {
						// Full-Size Replace
						echo "<td class='wr3x-fullsize-replace'><div class='wr3x-dragdrop'></div>";
						echo "</td>";
						// Full-Size Retina
						echo '<td id="wr3x-info-full-' . $post->ID . '" class="wr3x-image wr3x-info-full">';
						echo $this->core->html_get_basic_retina_info_full( $post->ID, $info );
						echo "</td>";
						// Full-Size Retina Upload
						echo "<td class='wr3x-fullsize-retina-upload'>";
						echo "<div class='wr3x-dragdrop'></div>";
						echo "</td>";
					}
					else
