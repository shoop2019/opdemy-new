...
class Meow_wr3x_Ajax {

	public $core = null;

	public function __construct( $core) {
		$this->core = $core;
		add_action( 'wp_ajax_wr3x_generate', array( $this, 'wp_ajax_wr3x_generate' ) );
		add_action( 'wp_ajax_wr3x_delete', array( $this, 'wp_ajax_wr3x_delete' ) );
		add_action( 'wp_ajax_wr3x_delete_full', array( $this, 'wp_ajax_wr3x_delete_full' ) );
		add_action( 'wp_ajax_wr3x_list_all', array( $this, 'wp_ajax_wr3x_list_all' ) );
		add_action( 'wp_ajax_wr3x_replace', array( $this, 'wp_ajax_wr3x_replace' ) );
		add_action( 'wp_ajax_wr3x_upload', array( $this, 'wp_ajax_wr3x_upload' ) );
		add_action( 'wp_ajax_wr3x_retina_upload', array( $this, 'wp_ajax_wr3x_retina_upload' ) );
		add_action( 'wp_ajax_wr3x_retina_details', array( $this, 'wp_ajax_wr3x_retina_details' ) );
	}
	...
	function check_nonce( $action ) {
		if ( !wp_verify_nonce( $_POST['nonce'], $action ) ) {
			echo json_encode(
				array (
					'success' => false,
					'message' => __( "Invalid API request.", 'wp-retina-3x' )
				)
			);
			die();
		}
	...
	function check_capability() {
		...
		if ( !current_user_can( $cap ) ) {
			echo json_encode(
				array (
					'success' => false,
					'message' => __( "You do not have permission to upload files.", 'wp-retina-3x' )
				)
			);
			die();
		}
	...
	function wp_ajax_wr3x_list_all( $issuesOnly ) {
		$this->check_nonce( 'wr3x_list_all' );
	...
	function wp_ajax_wr3x_delete_full( $pleaseReturn = false ) {
		if ( !$pleaseReturn ) $this->check_nonce( 'wr3x_delete_full' );
		$this->check_capability();
		...
		if ( !isset( $_POST['attachmentId'] ) ) {
			echo json_encode(
				array(
					'success' => false,
					'message' => __( "The attachment ID is missing.", 'wp-retina-3x' )
				)
			);
			die();
		}
		...
		echo json_encode(
			array(
				'results' => $results,
				'success' => true,
				'message' => __( "Full retina file deleted.", 'wp-retina-3x' )
			)
		);
		die();
	}

	function wp_ajax_wr3x_delete() {
		$this->check_nonce( 'wr3x_delete' );
		$this->check_capability();
		...
		$attachmentId = intval( $_POST['attachmentId'] );
		$results_full[$attachmentId] = $this->wp_ajax_wr3x_delete_full( true );
		...
		echo json_encode(
			array(
				...
				'success' => true,
				'message' => __( "Retina files deleted.", 'wp-retina-3x' )
			)
		);
		die();
	}

	function wp_ajax_wr3x_retina_details() {
		$this->check_nonce( 'wr3x_retina_details' );
		$this->check_capability();
		...
		echo json_encode(
			array(
				'result' => $info,
				'success' => true,
				'message' => __( "Details retrieved.", 'wp-retina-3x' )
			)
		);
		die();
	}

	function wp_ajax_wr3x_generate() {
		$this->check_nonce( 'wr3x_generate' );
		$this->check_capability();
		 ...
		 */
		do_action( 'wr3x_before_regenerate', $attachmentId );

		$this->core->delete_attachment( $attachmentId, false );

		// Regenerate the Thumbnails
		$regenerate = get_option( 'wr3x_regenerate_thumbnails', false );
		if ( $regenerate ) {

			/**
			 * @param $attachmentId ID of the attachment to generate thumbnails
			 */
			do_action( 'wr3x_before_generate_thumbnails', $attachmentId );
			 ...
			 */
			do_action( 'wr3x_generate_thumbnails', $attachmentId );
		}
		 ...
		 */
		do_action( 'wr3x_regenerate', $attachmentId );
		...
		echo json_encode(
			array(
				'results' => $results,
				'success' => true,
				'message' => __( "Retina files generated.", 'wp-retina-3x' )
			)
		);
		die();
	}
	...
	function check_get_ajax_uploaded_file() {
		...
		if ( empty( $file_info ) ) {
			...
			echo json_encode( array(
				'success' => false,
				'message' => __( "The file is not an image or the upload went wrong.", 'wp-retina-3x' )
			));
			die();
		}
		...
		if ( $filedata["ext"] == "" ) {
			...
			echo json_encode( array(
				'success' => false,
				'message' => __( "You cannot use this file (wrong extension? wrong type?).", 'wp-retina-3x' )
			));
			die();
		}
	...
	function wp_ajax_wr3x_upload( $checksNonce = true ) {
		if ( $checksNonce ) $this->check_nonce( 'wr3x_upload' );
		...
		try {
			 ...
			 */
			do_action( 'wr3x_before_upload_retina', $attachmentId, $retinafile );
			 ...
			 * @param $retinafile Path to the uploaded retina image
			 */
			do_action( 'wr3x_upload_retina', $attachmentId, $retinafile );
			...
			echo json_encode( array(
				'success' => false,
				'results' => null,
				'message' => __( "Error: " . $e->getMessage(), 'wp-retina-3x' )
			));
			die();
		}
		...
		echo json_encode( array(
			'success' => true,
			'results' => $results,
			'message' => __( "Uploaded successfully.", 'wp-retina-3x' ),
			'media' => array(
				'id' => $attachmentId,
	...
	function wp_ajax_wr3x_retina_upload() {
		require_once ABSPATH . 'wp-admin/includes/image.php';
		$this->check_nonce( 'wr3x_retina_upload' );
		$this->check_capability();
		...
		try {
			...
			$image->set_quality( get_option('wr3x_quality', 90) );
		...
		// Redirect to 'wr3x_upload'
		$_POST['attachmentId'] = $attachmentId;
		$this->wp_ajax_wr3x_upload( false );
	}

	function wp_ajax_wr3x_replace() {
		$this->check_nonce( 'wr3x_replace' );
		 ...
		 */
		do_action( 'wr3x_before_replace', $attachmentId, $tmpfname );
		 ...
		 */
		do_action( 'wr3x_replace', $attachmentId );
		...
		echo json_encode( array(
			'success' => true,
			'results' => $results,
			'message' => __( "Replaced successfully.", 'wp-retina-3x' )
		));
		die();
	}
