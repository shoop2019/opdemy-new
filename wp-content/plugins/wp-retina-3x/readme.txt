...
= 5.4.3 =
* Add: New hooks: wr3x_before_regenerate, wr3x_before_generate_thumbnails, wr3x_generate_thumbnails, wr3x_regenerate and wr3x_upload_retina.
...
* Important: A few options will be removed in the near future. Have a look at this: https://wordpress.org/support/topic/simplifying-wp-retina-3x-by-removing-options/.
...
= 5.1.4 =
* Add: wr3x_retina_extension, wr3x_delete_attachment, wr3x_get_pathinfo_from_image_src, wr3x_picture_rewrite in the API.
...
* Info: If you are using Lightroom, please have a look at my plugin for synchronizing your Lightroom to WordPress: https://meowapps.com/wplr-sync/. And if youi love my Retina plugin, please write a little review here: https://wordpress.org/support/plugin/wp-retina-3x/reviews/?rate=5#new-post. Thank you :)
...
* Add: Introduced $wr3x_extra_debug for extra developer debug (might be handy).
...
* Info: The serial for the Pro version can be bought at https://meowapps.com/wp-retina-3x. Thanks for all your support, the plugin is going to be 3 years old this year! :)
...
1. Upload the folder `wp-retina-3x` to the `/wp-content/plugins/` directory.
2. Activate the plugin through the 'Plugins' menu in WordPress.
3. Check the settings of WP Retina 3x in the WordPress administration screen.
4. Check the Retina Dashboard.
6. Read the tutorial about the plugin: <a href='https://meowapps.com/wp-retina-3x/tutorial/'>WP Retina 3x Tutorial</a>.

== Frequently Asked Questions ==

Users, you will find the FAQ here: https://meowapps.com/wp-retina-3x/faq/.

Developers, WP Retina 3x has a little API. Here are a few filters and actions you might want to use.

= Functions =
* wr3x_get_retina_from_url( $url ): return the URL of the retina image (empty string if not found)
* wr3x_get_retina( $syspath ): return the system path of the retina image (null if not found)

= Actions =
* wr3x_retina_file_added: called when a new retina file is created, 1st argument is $attachment_id (of the media) and second is the $retina_filepath
* wr3x_retina_file_removed: called when a new retina file is removed, 1st argument is $attachment_id (of the media) and second is the $retina_filepath

= Filters =
* wr3x_img_url: you can check and potentially override the $wr3x_img_url (normal/original image from the src) that will be used in the srcset for 1x
* wr3x_img_retina_url: you can check and potentially override the $wr3x_img_retina_url (retina image) that will be used in the srcset for 2x
* wr3x_img_src: you can check and potentially override the $wr3x_img_src that will be used in the img's src (only used in Pro version)
* wr3x_validate_src: the img src is passed; return it if it is valid, return null if it should be skipped
